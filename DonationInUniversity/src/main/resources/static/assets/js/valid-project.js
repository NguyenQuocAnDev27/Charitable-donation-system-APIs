...
// Kiểm tra khi người dùng nhập liệu vào trường goalAmount
document.getElementById('createGoalAmount').addEventListener('input', function () {
    const goalAmountInput = this;
    const goalAmount = goalAmountInput.value;
    ...
    if (!isNumber(goalAmount)) {
        goalAmountInput.classList.add('is-invalid');
        document.getElementById('create-goal-error').style.display = 'block'; // Hiển thị lỗi nếu Goal Amount không phải số
    } else {
        goalAmountInput.classList.remove('is-invalid');
        document.getElementById('create-goal-error').style.display = 'none'; // Ẩn thông báo lỗi nếu hợp lệ
    }
});


// Kiểm tra khi người dùng nhập liệu vào trường currentAmount
document.getElementById('createCurrentAmount').addEventListener('input', function () {
    const goalAmountInput = document.getElementById('goalAmount');
    const currentAmountInput = this;
    const goalAmount = goalAmountInput.value;
    const currentAmount = currentAmountInput.value;
    ...
    if (!isCurrentAmountValid) {
        currentAmountInput.classList.add('is-invalid');
        document.getElementById('create-amount-number-error').style.display = 'block'; // Hiển thị lỗi nếu Current Amount không phải là số
    } else {
        currentAmountInput.classList.remove('is-invalid');
        document.getElementById('create-amount-number-error').style.display = 'none'; // Ẩn thông báo lỗi số không hợp lệ nếu hợp lệ
    }
    ...
    if (isGoalAmountValid && isCurrentAmountValid) {
        ...
        if (currentAmountValue > goalAmountValue) {
            currentAmountInput.classList.add('is-invalid');
            document.getElementById('create-amount-goal-error').style.display = 'block'; // Hiển thị lỗi lớn hơn Goal Amount
        } else {
            currentAmountInput.classList.remove('is-invalid');
            document.getElementById('create-amount-goal-error').style.display = 'none'; // Ẩn thông báo lỗi nếu hợp lệ
        }
    } else {
        // Ẩn lỗi "greater than Goal Amount" nếu số không hợp lệ
        document.getElementById('create-amount-goal-error').style.display = 'none';
    }
});

// Kiểm tra lại trước khi form được submit
document.getElementById('createProject').addEventListener('submit', function (event) {
    const goalAmountInput = document.getElementById('createGoalAmount');
    const currentAmountInput = document.getElementById('createCurrentAmount');
    const goalAmount = goalAmountInput.value;
    const currentAmount = currentAmountInput.value;
    ...
    if (!isCurrentAmountValid) {
        currentAmountInput.classList.add('is-invalid');
        document.getElementById('create-amount-number-error').style.display = 'block'; // Hiển thị lỗi không phải số
        event.preventDefault(); // Ngăn form không được gửi đi nếu có lỗi
    }
    ...
    if (isGoalAmountValid && isCurrentAmountValid) {
        ...
        if (currentAmountValue > goalAmountValue) {
            currentAmountInput.classList.add('is-invalid');
            document.getElementById('create-amount-goal-error').style.display = 'block';
            event.preventDefault(); // Ngăn form không được gửi đi nếu Current Amount lớn hơn Goal Amount
        }
    }
});
document.getElementById('createEndDate').addEventListener('input', function () {
    ...
    if (startDate && endDate) {
        ...
        if (startDateValue > endDateValue) {
            endDateInput.classList.add('is-invalid');
            document.getElementById('create-date-error').style.display = 'block'; // Hiển thị lỗi nếu Start Date lớn hơn End Date
        } else {
            endDateInput.classList.remove('is-invalid');
            document.getElementById('create-date-error').style.display = 'none'; // Ẩn thông báo lỗi nếu hợp lệ
        }
    }
});
...
document.getElementById('createProject').addEventListener('submit', function (event) {
    ...
    if (startDate && endDate) {
        ...
        if (startDateValue > endDateValue) {
            endDateInput.classList.add('is-invalid');
            document.getElementById('create-date-error').style.display = 'block';
            event.preventDefault(); // Ngăn form không được gửi đi nếu có lỗi
        }
    }
});
...
document.getElementById('transferAmount').addEventListener('input', function () {
    ...
    if (!isNumber(goalAmount)) {
        goalAmountInput.classList.add('is-invalid');
        document.getElementById('transfer-amount-error').style.display = 'block'; // Hiển thị lỗi nếu Goal Amount không phải số
    } else {
        goalAmountInput.classList.remove('is-invalid');
        document.getElementById('transfer-amount-error').style.display = 'none'; // Ẩn thông báo lỗi nếu hợp lệ
    }
});